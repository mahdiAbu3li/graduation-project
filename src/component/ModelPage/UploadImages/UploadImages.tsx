  ...
  React.useEffect(() => {
    ...
    fetch(url, {
      ...
      .then((data) => {
        setIsLoading(false);
        if (data.length > 0) {
          console.log(data.length, "gett");
          setFiles(data);
        } else return false;
      });
  }, [modelId, values.data.token, x, values.data.id]);

  const uploadfile = async (data: any) => {
    const url =
      "https://graduationprojectt.herokuapp.com/api/images/predict/" + modelId; // + modelId;
    const requests = [];
    // for (var pair of data.entries()) {
    for (let i = 0; i < data.length; i++) {
      console.log(data[i], "i");
      var nx = new FormData();
      nx.append("images[]", data[i]);
      nx.append("user_id", values.data.id.toString());
